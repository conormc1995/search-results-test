...
//Remove old sheet
//Add new sheet
//Add queries to sheet
async function prepareSheets(queries) {
  //remove sheet
  ...
  const newSheet = await doc.addSheet({ title: "Test Results!" });

  //make new sheet
  await newSheet.loadCells(activeCells);
  let writeCell = newSheet.getCell(0, 0);

  for (let query of queries) {
    let x = queriers.indexOf(query);
    writeCell = newSheet.getCell(0, x);
    writeCell.value = query;
  }
}
